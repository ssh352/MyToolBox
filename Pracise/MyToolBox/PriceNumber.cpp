...
const double Double_Factor = 2 * MinValDetected;
	...
	PriceNumber& PriceNumber::operator=( double aVal )
	{
		m_PriceBase = 0;
		m_PriceExp = 0;
		m_PricePower = 10;
	...
	PriceNumber& PriceNumber::operator=( int32_t aVal )
	{
		m_PriceBase = aVal;
		m_PriceExp = 0;
		m_PricePower = 10;
	...
	bool PriceNumber::operator==( const double& aright )
	{
		return abs(double(*this) - aright ) < Double_Factor;
	}
	...
	PriceNumber& PriceNumber::operator+=( const PriceNumber& aright )
	{
		double lVal = double(*this) + double(aright);
		*this = lVal;
		return *this ;
	}

	PriceNumber& PriceNumber::operator+=( const double& aright )
	{
		return *this += PriceNumber(aright);
	}

	PriceNumber& PriceNumber::operator+=( const int& aright )
	{
		return *this += PriceNumber(aright);
	}
